...
| `GET/POST /api/v1/competitors` | 查询或创建 CRM 竞争对手台账；创建含 `Idempotency-Key` | `competitor:read` / `competitor:manage` |
| `POST /api/v1/competitors/{id}/actions/archive` | 归档竞争对手；请求含 `version` | `competitor:manage` |
...
| `CompetitorCreated/Archived` | Competitor | `competitorId,status,name` | 竞争信息台账、审计 |
