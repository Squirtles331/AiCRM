...
| 验收项 | 证据 | 当前状态 |
|---|---|---|
| 平台域、销售域、连接器、获客渠道和会话表名及字段冻结 | 数据字典 + V1-V23 | READY |
...
最近一次完整自动化验收：2026-09-11，`mvn -f java/pom.xml verify` 通过，共执行 26 个测试且无失败，覆盖至 V22。V23 已额外通过 `DatabaseStageGateTest`、`SchemaContractStaticTest` 和 18 项 `SalesApiV1IntegrationTest`：空库初始化、销售会话所有权、追加记录幂等、关闭重开、审计、Outbox、OpenAPI 与静态契约均无失败。发布前仍需在 V23 基线运行一次完整 `verify`。
