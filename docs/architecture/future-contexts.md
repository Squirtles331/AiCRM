...
第一里程碑只允许线索和客户闭环。下列对象仅冻结主责和引用方向，不代表字段级设计已通过；正式表必须在对应阶段门用新 Flyway 版本创建。

| 上下文 | 对象 | 主键与租户 | 跨域引用规则 |
|---|---|---|---|
| Catalog | 产品、分类、价格表、价格项、折扣规则 | Snowflake `BIGINT`；租户表必带 `tenant_id` | 销售/交易只存 `product_id/price_list_id` 和成交快照；不 FK 到 Catalog |
...
全局规则：金额 `NUMERIC(18,2)`，币种 ISO 4217，时间 UTC `TIMESTAMPTZ`；跨上下文只保存 ID，不建实体级 FK；跨域一致性依赖应用校验、Outbox 事件和对账。禁止在本阶段创建 `crm_product`、`crm_opportunity`、`crm_quote`、`crm_contract`、`crm_order`、`crm_delivery`、`crm_payment`、`crm_invoice` 或 `crm_ticket` 等正式表。
