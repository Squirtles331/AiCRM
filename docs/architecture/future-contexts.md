...
产品与价格目录已在阶段 2A 通过 V10 建表，商机管道已在阶段 2B 通过 V11 建表，报价根、版本和快照行已在阶段 2C 通过 V12 建表。下列其余对象仅冻结主责和引用方向，不代表字段级设计已通过；正式表必须在对应阶段门用新 Flyway 版本创建。
...
| Quotation | 报价、报价版本、报价行已实现；报价审批待后续迁移 | 报价根 ID + 版本 ID；租户隔离 | 引用商机/客户/价目表；每版固化产品、价格、税率和币种快照 |
