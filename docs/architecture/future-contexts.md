...
产品与价格目录已在阶段 2A 通过 V10 建表，商机管道已在阶段 2B 通过 V11 建表，报价根、版本和快照行已在阶段 2C 通过 V12 建表，审批定义及运行事实已在阶段 2D 通过 V13 建表。下列其余对象仅冻结主责和引用方向，不代表字段级设计已通过；正式表必须在对应阶段门用新 Flyway 版本创建。
...
全局规则：金额 `NUMERIC(18,2)`，币种 ISO 4217，时间 UTC `TIMESTAMPTZ`；跨上下文只保存 ID，不建实体级 FK；跨域一致性依赖应用校验、Outbox 事件和对账。除已实现的目录、商机和报价表外，禁止提前创建 `crm_contract`、`crm_order`、`crm_delivery`、`crm_payment`、`crm_invoice` 或 `crm_ticket` 等正式表。
