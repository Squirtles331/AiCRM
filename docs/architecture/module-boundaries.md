...
`ModuleDependencyTest` 自动检查领域纯净性和 Controller 不访问持久层。新增模块时必须扩充规则，CI 的 `mvn verify` 是合并门禁。聚合构建当前包含 `shared-kernel/platform/sales/catalog/trade/web/admin-boot` 七个业务模块及根聚合；`aicrm-sales` 内的线索、客户和商机通过各自 repository port 隔离持久化细节；`aicrm-trade` 只能通过 sales/catalog 的公开应用服务读取商机与价格主数据，不得引用其实体或持久层；不得重新引入横向 `common/dao/service/api` 模块、Mapper 实体或兼容 Controller。
