...
public class SalesTargetV1Controller {
    ...
    public ResponseEntity<ApiResponse<SalesTargetApiDtos.View>> create(@Valid @RequestBody SalesTargetApiDtos.CreateRequest request, @RequestHeader("Idempotency-Key") String key) {
        SalesTarget target = commands.create(ActorContext.require(), new SalesTargetCommands.Create(request.name(), request.targetUserId(), request.metric(), request.periodFrom(), request.periodTo(), request.targetValue()), key);
        return ResponseEntity.status(HttpStatus.CREATED).body(success(view(new SalesTargetReadService.TargetDetail(target, null))));
    }
    ...
    private static SalesTargetApiDtos.View view(SalesTargetReadService.TargetDetail detail) { SalesTarget t=detail.target(); SalesTargetResult r=detail.result(); return new SalesTargetApiDtos.View(String.valueOf(t.id()),t.targetNo(),t.name(),String.valueOf(t.targetUserId()),t.metric().name(),t.periodFrom(),t.periodTo(),t.targetValue(),t.status().name(),t.version(),t.createdAt(),t.updatedAt(),r==null?null:new SalesTargetApiDtos.ResultView(String.valueOf(r.id()),r.actualValue(),r.achievementRate(),r.calculationSnapshot(),String.valueOf(r.confirmedBy()),r.calculatedAt(),r.confirmedAt())); }
